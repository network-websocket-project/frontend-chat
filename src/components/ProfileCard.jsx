...
const ProfileCard = ({ h, group, name = "Demo", status = false, onClick, large, profile, noti, avatar }) => {
    // console.log('eiei');
    return (
        <>
            <div className={`${large ? "h-20" : "h-12"} relative flex flex-row ${large ? "items-start" : "items-center"} gap-x-${large ? 8 : 4}  font-montserrat min-w-fit cursor-pointer`} onClick={onClick}>
                <div className="relative">
                    {group ? <img src={groupProfile} className={`${large ? "h-[64px]" : "h-[32px]"}`} /> : <img src={avatar ? avatar : defaultProfile} className={`${large ? "h-[64px] w-[64px]" : "h-[32px] w-[32px]"}`} />}
                    {!group && !profile && <img src={status ? online : offline} className={large ? "w-4 absolute bottom-1 right-0" : "absolute bottom-0 right-0"} />}
                </div>
