...
const ChatRoomPage = () => {
    ...
    useEffect(() => {
        const fetchAllUser = async () => {
            ...
            chatListTest.forEach((chat) => {
                chat["isGroup"] = false;
                // chat["isOnline"] = false;
                // console.log(initialOnline, chat._id);
                if (initialOnline.includes(chat._id)) {
                    chat["isOnline"] = true;
                }
                else {
                    chat["isOnline"] = false;
        ...
        if (authCtx.userInfo && !socketConnected) {
            ...
            socket.on("connected", (users) => {
                ...
                for (const [key, value] of Object.entries(users)) {
                    setInitialOnline((prev) => [...prev, value._id]);
                }
            ...
            socket.on("message received", async (newMessageRecieved) => {
                ...
                if (
                    ...
                    const message = {
                        ...
                        avatar: newMessageRecieved.sender.avatar
                    };
    ...
    const sendMessage = async (event) => {
        if (event.key === "Enter" && newMessage) {
            ...
            const message = { sender: data.sender.nickname, content: data.content, avatar: data.sender.avatar };
            setMessageList((prev) => [...prev, message]);
                        ...
                        {chatList.map((chat, i) => {
                            return (
                                <ProfileCard
                                    ...
                                    status={chat.isOnline ? chat.isOnline : false}
                                    key={i}
                                    onClick={chatChangeHandler.bind(null, chat)}
                                    avatar={chat.avatar ? chat.avatar : null}
                                />
                            );
                        })}
                    </div>
                ...
                {selectChat && (
                    <div className="w-full bg-[#151223] h-[90vh] px-8 pt-4 pb-8 flex flex-col overflow-y-scroll">
                        <ProfileCard
                            ...
                            large
                            status={selectChat.isOnline ? selectChat.isOnline : false}
                            avatar={selectChat.avatar ? selectChat.avatar : null}
                        />
                        <div
                            ...
                            {messageList.map((message, i) => {
                                ...
                                return (
                                    <div className="flex flex-row items-center gap-x-2" key={i}>
                                        <img
                                            src={message.avatar ? message.avatar : defaulProfile}
                                            className={`w-8 ${message.sender === authCtx.userInfo.nickname
                                                ? "collapse"
                                                : "visible"
                                                }`}
                                        />
                                        ...
                                        >
                                            {selectChat.isGroup && message.sender !== authCtx.userInfo.nickname ? message.sender + ": " + message.content : message.content}
                                        </div>
